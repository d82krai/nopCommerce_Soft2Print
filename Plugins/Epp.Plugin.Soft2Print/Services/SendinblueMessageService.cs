...
namespace Epp.Plugin.Soft2Print.Services
{
    ...
    public class SendinblueMessageService : WorkflowMessageService
    {
        ...
        private async Task SendSmsNotificationAsync(MessageTemplate messageTemplate, IEnumerable<Token> tokens)
        {
            ...
            var sendinblueSettings = await _settingService.LoadSettingAsync<Soft2PrintSettings>(storeId ?? 0);
            ...
            var sendSmsForThisMessageTemplate = await _genericAttributeService
                .GetAttributeAsync<bool>(messageTemplate, Soft2PrintDefaults.UseSmsAttribute);
            if (!sendSmsForThisMessageTemplate)
                return;

            //get text with replaced tokens
            var text = await _genericAttributeService.GetAttributeAsync<string>(messageTemplate, Soft2PrintDefaults.SmsTextAttribute);
            if (!string.IsNullOrEmpty(text))
                text = _tokenizer.Replace(text, tokens, false);

            //get phone number send to
            var phoneNumberTo = string.Empty;
            var phoneType = await _genericAttributeService.GetAttributeAsync<int>(messageTemplate, Soft2PrintDefaults.PhoneTypeAttribute);
            switch (phoneType)
            {
                case 0:
                    //merchant phone
            ...
            string fromEmail = null, string fromName = null, string subject = null)
        {
            ...
            var templateId = await _genericAttributeService.GetAttributeAsync<int?>(messageTemplate, Soft2PrintDefaults.TemplateIdAttribute);
